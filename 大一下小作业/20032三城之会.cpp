...
node node_list[50005];
edge edge_list[100008];
bool visited[50005]{false};
int depth[50005]{0};
int f[50005][21];
int ancestor_weight_sum[50005];
...
//    }
    ...
    for (int i = 0; i < q; ++i) {
        ...
        int result;
        if (lca_yz == lca_xy) {
            result = ancestor_weight_sum[x] + ancestor_weight_sum[z] +
                     ancestor_weight_sum[y] - 2*ancestor_weight_sum[LCA(lca_zx, y)] - ancestor_weight_sum[lca_zx];
        } else if (lca_yz == lca_zx) {
            result = ancestor_weight_sum[x] + ancestor_weight_sum[y] +
                     ancestor_weight_sum[z] - 2*ancestor_weight_sum[LCA(lca_xy, z)] - ancestor_weight_sum[lca_xy];
        } else {
            result = ancestor_weight_sum[y] + ancestor_weight_sum[z] +
                     ancestor_weight_sum[x] - 2*ancestor_weight_sum[LCA(lca_yz, x)] - ancestor_weight_sum[lca_yz];
        }
        printf("%d%c", result, '\n');
    }
    return 0;
