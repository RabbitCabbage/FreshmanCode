 ...
 * File: program.h
 * ---------------
 * This interface exports a Program class for storing a BASIC//它会产生一个存有程序的program对象
 * program.
 */
...
/*
 * This class stores the lines in a BASIC program.  Each line//他按照行序存下来原本的程序
 * in the program is stored in order according to its line number.
 * Moreover, each line in the program is associated with two
 * components:并且每一行都有两个组成部分，一个是源码，一个是返回一个statement指针，是剖析过的语句
 *
 ...
 * ----------------------------------------------------
 * Adds the parsed representation of the statement to the statement//把parsed之后的statement放入特定的行数
 * at the specified line number.  If no such line exists, this
 * method raises an error.  If a previous parsed representation
 * exists, the memory for that statement is reclaimed.如果之前已经有国set了就回收
 */
 ...
 * ----------------------------------------------------------------
 * Retrieves the parsed representation of the statement at the//得到特定行数的parsed之后的语句
 ...
 */
    ...
    bool require(int line_num);

private:

// Fill this in with whatever types and instance variables you need
    map<int,string>program_lines;
